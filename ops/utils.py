...
def save_bias(net, checkpoint_dir, epoch, iters):
    ...
    for layer_id in range(1, 5):
        ...
        for i, b in enumerate(blocks):
            #bias_data[idx] = blocks[i].conv1.feat_bias.cpu().detach().numpy()
            #weight_data[idx] = blocks[i].conv1.feat_weight.cpu().detach().numpy()
            bias_data[idx] = blocks[i].conv1.buffer[0]
            weight_data[idx] = blocks[i].conv1.buffer[1]
            idx += 1
    w_save_path = os.path.join(checkpoint_dir, 'data', '%d_%d_weight.npz' % (epoch, iters))
    b_save_path = os.path.join(checkpoint_dir, 'data', '%d_%d_bias.npz' % (epoch, iters))
