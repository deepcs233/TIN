...
from ops.utils import AverageMeter, accuracy, save_bias
from ops.calculate_map import *
...
def main():
    ...
    if args.tin:
        full_arch_name += '_tin{}'.format(args.shift_div)
    if args.temporal_pool:
        full_arch_name += '_tpool'
    args.store_name = '_'.join(
        ['TSM', args.dataset, args.modality, full_arch_name, args.consensus_type, 'segment%d' % args.num_segments, str(args.lr),
         'e{}'.format(args.epochs), args.store_name])
    ...
    if args.resume:
        ...
        if args.resume_path:
            ...
            resume_path = '%s/%s/ckpt.pth.tar' % (args.root_model, args.store_name)
        if os.path.isfile(resume_path):
            print(("=> loading checkpoint '{}'".format(resume_path)))
            checkpoint = torch.load(resume_path)
            args.start_epoch = checkpoint['epoch']
            best_prec1 = checkpoint['best_prec1']
            incompatibleKeys = model.load_state_dict(checkpoint['state_dict'], strict=False)
...
def adjust_learning_rate(optimizer, epoch, lr_type, lr_steps, epoch_float=-1):
